...
allCards.forEach(function(card) {
  ...
  if (currentFaceupCards < 2) {
    if ((card.classList.contains('open') && card.classList.contains('show') && card.classList.contains('match')) == false) {
      currentFaceupCards++;
      card.classList.add('open','show');
      faceupArray.push(card);
      //if they Match
      if (currentFaceupCards === 2){
      if ((faceupArray[0].querySelector('i').classList.value == faceupArray[1].querySelector('i').classList.value)==false) {
